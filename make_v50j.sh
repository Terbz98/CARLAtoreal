#!/bin/bash
# v50j: rebuild the sunny baseline chain with the vehicle-colour bug fixed.
#
...
#
# Rebuilds v50b then v50d from the untouched v50 deliveries, as v50j/v50j, alongside the originals.
set -u
...
OUT=$BASE/pix2pixHD/results/mp4/v50j
LOG=$BASE/pix2pixHD/checkpoints/v50j_log.txt
AMOUNT=${AMOUNT:-0.55}
mkdir -p "$OUT"
echo "=== V50J sunny colour fix $(date) ===" > "$LOG"
...
w.release(); print('  sharpened %d frames'%n)" >> "$LOG" 2>&1
  ...
  DN=${T}_sunny_vp55_v50j_FINAL_1920_visionpilot.mp4
  $CE python3 -u -c "
import cv2
...
w.release(); print('  FINAL_1920 %d frames'%n)" >> "$LOG" 2>&1
  ...
  mkdir -p "$DEST/vp_input_1024" "$DEST/logs_v50j"
  $CE python3 -u -c "
import cv2
...
w.release()" >> "$LOG" 2>&1
  ...
  ( cd $PERCEPTION_ROOT/build && ./record_carla.sh "$DEST/vp_input_1024/$DN" \
      "$SPD" "$DEST/calibrated/$DN" "$DEST/logs_v50j/${T}_sunny.log" ) >> "$LOG" 2>&1 \
    || echo "  $T: VP run failed" >> "$LOG"
  G=$DEST/gt/${T}_sunny_gt.json
  [ -s "$G" ] && $CE python3 $BASE/score_vp.py "$G" "$DEST/logs_v50j/${T}_sunny.log" \
      --json "$DEST/logs_v50j/${T}_sunny_score.json" >> "$LOG" 2>&1
  rm -f "$OUT/${T}_col.avi" "$OUT/${T}_bld.avi" "$OUT/${T}_sharp.avi" "$OUT/${T}_ds.avi"
  echo "  $T DONE -> $DN" >> "$LOG"
done

$CE python3 $BASE/flicker_report.py v50d v50j --frames 300 >> "$LOG" 2>&1
bash $BASE/organise_calibrated.sh >> "$LOG" 2>&1
echo "=== V50J DONE $(date) ===" >> "$LOG"
